...
def mail(a: float, f: float):  # a--> Amazon Price , b--> Flipkart Price

    Setserver=smtplib.SMTP('smtp.gmail.com',587)    #establish connection gmail and our server
    Setserver.ehlo()           
    Setserver.starttls()                            #encrpts the email
    Setserver.ehlo()

    Setserver.login('senders email', 'password')

    Subject ='Price Got Dropped!!!'
    #Change the Link in the Body According the required Product
